...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--turboquant"},
        string_format("enable TurboQuant KV cache quantization (default: %s)", params.use_turboquant ? "true" : "false"),
        [](common_params & params) {
            params.use_turboquant = true;
        }
    ).set_env("LLAMA_ARG_TURBOQUANT").set_examples({LLAMA_EXAMPLE_COMMON}));
    add_opt(common_arg(
