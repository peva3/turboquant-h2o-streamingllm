...
private:
     ...
     struct kv_layer {
          ...
          uint8_t* key_quantized_data = nullptr;
          uint8_t* value_quantized_data = nullptr;
        ...
        float* key_vector_norms = nullptr;
        float* value_vector_norms = nullptr;
        size_t norms_capacity = 0;

        // H2O attention score accumulation
        float* attention_scores = nullptr;  // Cumulative attention scores per position
        size_t attention_scores_capacity = 0;
           ...
           : il(il_), k(k_), v(v_), k_stream(std::move(k_stream_)), v_stream(std::move(v_stream_)),
             key_quantized_data(nullptr), value_quantized_data(nullptr),
        ...
        // Destructor to free quantized buffers and norm arrays
        ~kv_layer() {
            delete[] key_quantized_data;
            delete[] value_quantized_data;
            delete[] key_vector_norms;
            delete[] value_vector_norms;
