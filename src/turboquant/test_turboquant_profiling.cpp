...
int main() {
    ...
    for (int i = 0; i < num_vectors; ++i) {
        turboquant::apply_rotation(turboquant::generate_random_rotation(embedding_dim, 42), 
                                  test_vectors[i].data(), 
    ...
    for (size_t i = 0; i < test_vectors.size(); ++i) {
        // Generate a new random projection for each call (not ideal but works for profiling)
        std::vector<float> random_proj(embedding_dim);
        std::mt19937 proj_gen(42 + static_cast<int>(i)); // Different seed for each vector
        std::normal_distribution<float> proj_dist(0.0f, 1.0f / std::sqrt(embedding_dim));
        for (int j = 0; j < embedding_dim; ++j) {
            random_proj[j] = proj_dist(proj_gen);
        }
        // Normalize projection vector
        float norm = 0.0f;
        for (float val : random_proj) {
            norm += val * val;
        }
        norm = std::sqrt(norm);
        for (float& val : random_proj) {
        ...
        float qjl_bit = turboquant::qjl_1bit(test_vectors[i].data(), random_proj, embedding_dim);
