...
std::pair<std::vector<uint8_t>, float> TurboQuantKVCache::quantize(const float* vector, int dim) {
    ...
    for (int i = 0; i < dim; ++i) {
        normalized[i] = vector[i] / vector_norm;
    }
    ...
    // Return indices and QJL bit (norm is stored separately in kv_layer)
    return {mse_indices, qjl_bit};
}
