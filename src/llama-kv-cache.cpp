...
if (use_turboquant && k) {
    ...
    size_t bytes_per_vector = (bits_per_vector + 7) / 8;
        ...
        new_layer.bytes_per_vector = bytes_per_vector;
            ...
            new_layer.key_vector_norms = new float[kv_size]();
            new_layer.value_vector_norms = new float[kv_size]();
            // Initialize norms to 1.0 to avoid issues with zero norms
            for (int i = 0; i < kv_size; i++) {
                new_layer.key_vector_norms[i] = 1.0f;
                new_layer.value_vector_norms[i] = 1.0f;
            }
        ...
        new_layer.attention_scores = new float[kv_size]();
    }
...
turboquant::H2OAttentionAccumulator* llama_kv_cache::create_h2o_accumulator_if_needed(int32_t il, int n_head, int n_kv) {
    ...
    return new turboquant::H2OAttentionAccumulator(layer.attention_scores, n_kv, n_head);
}
...
ggml_tensor * llama_kv_cache::cpy_k(ggml_context * ctx, ggml_tensor * k_cur, ggml_tensor * k_idxs, int32_t il, const slot_info & sinfo) const {
    ...
    if (layers[ikv].use_turboquant && layers[ikv].key_turboquant && layers[ikv].key_quantized_data) {
        ...
        for (int64_t t = 0; t < n_tokens; t++) {
            ...
            memcpy(layers[ikv].key_quantized_data + pos * bytes_per_vector, packed.data(), std::min(n_bytes, bytes_per_vector));
...
ggml_tensor * llama_kv_cache::cpy_v(ggml_context * ctx, ggml_tensor * v_cur, ggml_tensor * v_idxs, int32_t il, const slot_info & sinfo) const {
    ...
    if (layers[ikv].use_turboquant && layers[ikv].value_turboquant && layers[ikv].value_quantized_data) {
        ...
        for (int64_t t = 0; t < n_tokens; t++) {
            ...
            size_t pos = sinfo.idxs[0].size() > (size_t)t ? sinfo.idxs[0][t] : (size_t)t;
            memcpy(layers[ikv].value_quantized_data + pos * bytes_per_vector, packed.data(), std::min(n_bytes, bytes_per_vector));
            if (pos < layers[ikv].norms_capacity) {
        ...
        for (uint32_t s = 0; s < n_stream; s++) {
            ...
            for (uint32_t pos = attention_sink_count; 
                ...
                if (pos < layer.attention_scores_capacity) {
                    float score = layer.attention_scores[pos];
        ...
        for (uint32_t s = 0; s < n_stream; s++) {
            ...
            for (uint32_t block = 0; block < num_blocks; block++) {
                ...
                for (uint32_t pos = block_start; pos < block_end; pos++) {
                    if (pos < layer.attention_scores_capacity) {
                        total_score += layer.attention_scores[pos];
                        count++;
                    }
                }
